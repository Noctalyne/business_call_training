...
export const StoryCont = createContext(); 
...
const storyData = [
    ...
    {
        id: 0,
        title: `Bonjour, ${userData.prenom} ${userData.nom} étudiante à l'EPSI, je cherche à joindre ${msg}, s'il vous plait.`,
...
export const ContextDataProvider = ({children}) => {
    const [userDataCont, setUserData] = useState(userData);
    const storyDataCont = useState(storyData);

    return (
        <StoryCont.Provider value={{ userDataCont, setUserData , storyDataCont }}>
            {children}
        </StoryCont.Provider>
    )
} 